...
def get_fgt_acc(score_mel, score_acc, perf_mel, perf_acc, p, sec_p_beat):
    ...
    for i in range(0, CM):
        ...
        if i == 0 or i == CM-1:
            ...
            pre_pcevt_t = np.mean(np.nonzero(perf_acc_aligned_filled[cevts_stix[i-1]:cevts_stix[i], 2])[0])
            next_pcevt_t = np.mean(np.nonzero(perf_acc_aligned_filled[cevts_stix[i]:cevts_stix[i+1]+len(cevts[i+1]), 2])[0])
    ...
    if d_rowix != []:
        perf_acc_fixed_before = perf_acc
