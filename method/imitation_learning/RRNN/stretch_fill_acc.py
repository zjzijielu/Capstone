...
def stretch_fill_acc(score_mel, score_acc, perf_mel, perf_acc, p, sec_p_beat):
    ...
    for i in range(len(oaccix)):
        ...
        perf_cevt_accix = np.setdiff1d(score_cevt_accix, oaccix)
        # if yes, use the perf_acc average time as the timing. Also use
        # the average velocity as velocity
        if perf_cevt_accix.shape[0] != 0:
            perf_acc_aligned[idx, 1] = np.ceil(np.sum(perf_acc_aligned[perf_cevt_accix, 1]) / len(perf_cevt_accix))
            # if idx == 61 or idx == 68:
            #     print("perf_cevt_accix", perf_cevt_accix)
            #     print("index", idx, "computed median", np.median(perf_acc_aligned[perf_cevt_accix, 2:4], axis=0))
            ...
            if len(non_o_melix) < MIN_MEL_REGNUM:
                ...
                for j in range(len(y)):
                    ...
                    if non_o_local_accix.shape[0] != 0:
                        y[j] = np.median(perf_acc_aligned[non_o_local_accix + local_accix[0], 2]) # NOTSURE
