...
def stretch_follow_acc(ref_mel, ref_acc, perf_mel, p, score_mel, score_acc, flag_anchor,sec_p_beat=1, flag_self=0, perf_acc_fgt=None):
    ...
    # print("p", p, "sec_p_beat", sec_p_beat)
    # print("score mel starting time: ", st)
    # raise ValueError
    sparse_ix = []
    ...
    for i in range(begin_ix, M):
        ...
        if len(non_o_melix) < p:
            non_o_melix = np.where(perf_mel_aligned[0:pre_melix, 0])[0][-p:]
            sparse_ix.append(i)
        ...
        # keep the pitch
        # print("ref_acc", ref_acc[i, 0])
        rslt_stretch[i, 0] = ref_acc[i, 0]
        assert(ref_acc[i, 0] != 0)
